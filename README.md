...
| `silprespec-emulator` | 前置规范效果实验台。按方法论（08a 前置规范>后置验证 / 08b 填空的边界 / 08c 槽位的减法 / 09b 穷举一致性）提供 **8 种前置规范方式**，用户按场景选一种或多种、自由裁剪，对输入**真实执行**（LLM 真填空），观测填入了什么、重试次数、撑满失败、重现性——验证"前... |

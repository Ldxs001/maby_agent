...
def generate_novel_article(outline, user_orders, rag_options, llm_client,
    ...
    # b14 修复：模型/质检配置统一读 config.json（config_mgr 最新）——不再读 session.config
    # 孤儿快照（init_session 时拷一次、永不更新 → 用户换模型后旧 session 串行调度仍加载旧模型"27B 幽灵"）
    try:
        from ..config_manager import ConfigManager as _CM
        _cfg_all = _CM().get_all()
    except Exception:
        _cfg_all = (state_mgr._state or {}).get("config") or {}
    _checks = _cfg_all.get("novel_checks", {}) or {}
    ...
    if _serial:
        try:
            ...
            # b14 修复：串行 key 统一读 config.json（与 llm_client 同源）——不再读 session.config
            # 孤儿快照（旧 session 的旧模型 key → 换模型后串行调度先加载旧模型"27B 幽灵"）
    ...
    for idx, section in enumerate(outline.get("sections", []), 1):
        ...
        if section.get("status") == "pending":
            ...
            try:
                ...
                # 规划失败不静默跳章（历史根因：失败 continue → 前置章缺失且用户无感知，
                # 出现"L03 没规划、L04 照写"的跳章缺章）：
                # 置 planning_failed 标记（前端可据此提示）+ 明确状态文本 + 停止整个生成流程，
                # 用户重试「开始生成」时该章（pending）会被优先重新规划。
                state_mgr.update_section(sid, {"status": "pending", "planning_failed": True})
                state_mgr.set_status_text(f"{chapter_id} 子结构规划失败，已停止后续章节（原因: {e}）。请修复后重试「开始生成」")
                md_parts.append(f"\n\n## {section['title']}\n\n> **子结构规划失败**: {e}\n\n")
                _logger.warning(f"{chapter_id} 子结构规划失败，停止生成流程（不再跳过后续章）: {e}")
                break

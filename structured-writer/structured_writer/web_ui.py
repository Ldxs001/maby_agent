...
class StructuredWriterHandler(BaseHTTPRequestHandler):
    ...
    def _handle_generate(self):
        ...
        # 应用 RAG 勾选（持久化到 outline——此前只进请求体不落盘，重渲染后勾选消失但写作仍生效，
        # 用户"状态没持久、UI没统一"；与 titles/key_sections 同款写回）
        rag_apply = data.get("rag", {})
        if rag_apply:
            for s in outline.get("sections", []):
                if s["id"] in rag_apply:
                    s["rag"] = rag_apply[s["id"]]
        ...
        # 持久化显式顺序选择（sub.order——此前 sub_orders 只用于排序不落盘，
        # 重渲染 selected 按 id 后缀猜 → 调整顺序后 UI 与实际错位；同 rag/aux 型"状态不持久"）
        sub_orders_p = data.get("sub_orders", {})
        if sub_orders_p:
            for s in outline.get("sections", []):
                for ss in s.get("sub_sections", []):
                    if ss["id"] in sub_orders_p:
                        ss["order"] = sub_orders_p[ss["id"]]

        # 持久化辅助知识（sub.aux_knowledge——此前 aux 只进请求体不落盘，
        # 弹窗回显（读 ss.aux_knowledge）永远空；同 rag 型"状态不持久"）
        aux_apply = data.get("aux_knowledge", {})
        if aux_apply:
            for s in outline.get("sections", []):
                for ss in s.get("sub_sections", []):
                    if ss["id"] in aux_apply:
                        ss["aux_knowledge"] = aux_apply[ss["id"]]
  ...
  sections.forEach((s, i) => {
    ...
    subs.forEach(ss => {
      const subOpts = ['', ...Array.from({length: subCount}, (_, i) => `s${i+1}`)]
        .map(v => `<option value="${v}" ${(ss.order ? (ss.order === v) : (v === '' && subs.indexOf(ss) === 0)) ? 'selected' : ''}>${v === '' ? '自动' : v}</option>`).join('');
      subHTML += `
    ...
    secHTML += `
      <div class="section-card" data-sid="${s.id}">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;width:100%">
          ...
          ${readOnly ? '' : `<label class="sc-rag"><input type="checkbox" class="sc-rag-cb" ${s.rag && s.rag.enabled ? 'checked' : ''} onchange="onRagToggle(this, '${s.id}')" ${!ragOnline ? 'disabled title="RAG未连接"' : ''}> RAG</label>` + (ragOnline && Array.isArray(ragKbs) ? `<select class="sc-kb" style="display:${s.rag && s.rag.enabled ? '' : 'none'};width:120px;font-size:12px" onchange="collectOutlineData()">${'<option value="">自动KB</option>' + ragKbs.map(k => '<option value="' + k + '"' + (s.rag && s.rag.kb === k ? ' selected' : '') + '>' + k + '</option>').join('')}</select>` : '')}

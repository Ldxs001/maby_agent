...
都在这个项目的子文件夹里。项目记住节目名、副标题、风格倾向、音色、称呼与期号，出片时自动往后排：
...
| 节目名 / 副标题 | 整档节目共用一份，卡片上「节目名/副标题」改一次，往后各期画面都跟着变；节目名就是画面上最大的那行 |
| 只出一集 | 照样立项——规划方式选「单集」，不排图、出完即完结。**没有第二条路可走** |
| 项目设定 vs 全局配置 | 项目优先，且只作用于本次运行，不回写全局 |

节目名与副标题**不出现在配置页**——它们是项目设定，不是全局默认值。改的地方是项目
卡片上的「节目名/副标题」。
...
共 21 条（生成阶段 13 条、产物阶段 8 条；其中 fail 17、warn 4，另有 soft 1），分 fail 与 warn 两级。
...
| 阶段 | 门禁 |
|------|------|
| 脚本生成·形式 | JSON 合法、字段完整、同一人连续句数（上限取自素材类型的范式卡）、句长区间、单句时长、情绪标签、情绪档位（不写心情的文体不许出现心情词）、禁用词、可朗读（不含 emoji、图标、不可见字符、网址、命令参数、排版符号）、片头片尾 |
...
| | 脚本阶段 | 合成阶段 |
|---|---|---|
| 判什么 | 生成阶段那 13 条（形式项 + 总时长软项 + 内容检） | 产物阶段那 8 条 |
...
输出是一份补丁（`{edits: [{index, text, emotion}]}`），回来按句号原地替换，**全篇行数一个
字不动**。行数不变是这条路的根基：门禁报的「第 N 句」在补丁前后指的是同一行，一旦允许
增删，编号全漂，就得回头重编一遍。

「不许增删」不靠提示词劝，靠补丁的结构里根本写不出来——只有「改哪一句」和「改成什么」，
没有「加一句」和「删一句」。说话人也不在补丁里：它归代码管（`enforce_max_run` 压连续句、
`pin_intro_outro` 写死首末句），给模型留这道口子等于让格式轴上出现两个说话的人。
...
**素材喂多少由素材容量算出来，不由常数定。** 内容检与定点修补从前都把素材截到前 6000 字——
生产里一期素材约 2 万字，只喂进 30%；落在后面的依据在模型眼里就是「不存在」，语义检于是
把本来正确的句子判成编造，再交给定点修补把它改坏。现在按素材容量喂：**容量 = 成稿目标 × 1.25 ×
压缩档**（偏移 1.25 只进算式、不改变参照系的「1」，与排图体检同一把尺），扣掉估算余量与提示词
模板的占用，剩下的才是素材的；后端窗口够不够不靠程序猜——任务开始时日志会给出具体数字
并提醒「请保证后端上下文窗口 ≥ 输出预算 + 素材折算」。字符与 token 的折合比由后端回传的
`prompt_tokens` 反标，不写死经验值——中文、数字、标点的折合比差得远。

**放不下的时候，两条路分工不同。** 写作与定点修补不分批：脚本是单篇产物（片头尾写死、
承诺链跨全篇、总字数由目标时长反推），分批写要引入规划层与段间状态传递，那是另一套架构。
素材超出预算时改用凝缩（每节的逻辑骨架）顶替，这是有损的，日志与素材头部都写明。内容检
则**分批核**：按预算切段、相邻批重叠 1000 字、每批带整篇脚本，合并规则只有一句——同一句
要在每一批里都被报出来才算真问题（某句的依据只落在某一批里，那一批当然找不到它，撤销它
才对；每一批都找不到，才等价于全文里找不到）。批数超过 8 批判「未核」交人工，不把没核的
那部分记成「核过且没问题」。

**成稿规划走分段生成，治字数漂移。** 整篇一次生成时模型没有全局计数器，实测同一管线
一期超目标 4 倍、另一期只写六成。成稿规划的项目有地图与各节凝缩，脚本这一步先按凝缩
分组规划（模型不报数字，字数配额由程序按凝缩占比分），再逐段生成、逐段按字数核账
（容差与总时长门禁同一把尺），差额滚入下段配额——误差逐段吸收，总账咬住目标。
...
**最大的一行是节目名，不是期标题。** 背景与封面自上而下同一条层级：
...
│   ├── ingest.py           素材导入与锚点切章
│   ├── script_engine.py    脚本生成、格式收束、片头尾写死、门禁、锁文件

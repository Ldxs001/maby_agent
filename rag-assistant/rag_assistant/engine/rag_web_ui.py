...
function toggleNLIOutput(){if(!hasNLIModel){toast('未检测到已下载的 NLI 模型，请先下载','error');return;}fetch('/api/nli/output-toggle',{method:'POST'}).then(function(r){return r.json()}).then(function(d){if(d.success){toast('NLI 输出校验:'+(d.enabled?'启用':'禁用'));setTimeout(function(){location.reload()},200)}else toast(d.error,'error')});}
...
function onSlotsChange(forgetPreset) {
  ...
  _slot_save_timer = setTimeout(function() {
    ...
    var body = JSON.stringify(slots);
    if (!forgetPreset) {
      var sel = document.getElementById('prompt-preset');
      var presetKey = sel ? sel.value : '';
      // 同时保存选中的预设 key
      fetch('/api/prompt/presets/select', {
        method: 'POST', headers: {'Content-Type':'application/json'},
...
def generate_html():
    ...
    slots = load_slots()
    current_preset = get_selected_preset()
    ...
    if builtins:
        ...
        for key, p in builtins.items():
            ...
            preset_options += f'<option value="{key}" data-cite="{cite}" data-style="{style}" data-fallback="{fb}" data-builtin="1"{' selected' if key == current_preset else ''}>{p["label"]}</option>'
    ...
    if customs:
        ...
        for key, p in customs.items():
            ...
            style = _html_attr(s.get("output_style", ""))
            fb = _html_attr(s.get("fallback", ""))
            preset_options += f'<option value="{key}" data-cite="{cite}" data-style="{style}" data-fallback="{fb}" data-builtin="0"{' selected' if key == current_preset else ''}>{p["label"]}</option>'
        preset_options += '</optgroup>'
    ...
    # 过滤掉重排序/NLI模型，只保留真正的嵌入模型（供 KB 嵌入模型选择器使用）
    ...
    def _mlist(role, models_def, current_path=""):
        ...
        for m in models_def:
            ...
            dim_str = f' | {MODEL_DIMENSION_MAP.get(mid)}维' if role=="embedding" and MODEL_DIMENSION_MAP.get(mid) else ''
            rows.append(f'<div style="display:flex;align-items:center;gap:10px;padding:6px 0;border-bottom:1px solid #eee;"><input type="radio" name="{role}-model" value="{mid}" id="{role[:3]}-{mid}" {checked} {hdl} style="flex-shrink:0;"><label for="{role[:3]}-{mid}" style="flex:1;font-size:13px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="{mid}">{"[嵌入] " if role=="embedding" else "[重排序] " if role=="rerank" else "[路由] " if role=="fb" else "[NLI] "}{mid} <span style="color:#888;font-size:11px;">({m["size_mb"]}MB{dim_str})</span><span class="avail-indicator" data-model-id="{mid}" style="display:none;"></span></label><span style="font-size:11px;flex-shrink:0;color:{"#3B6D11" if ok else "#888"};">{st}</span>{bt}</div>')
        return "\n".join(rows)
    emb_model_html = _mlist("embedding", RECOMMENDED_MODELS, cfg.get("embedding",{}).get("model_path",""))
    # 追加本地发现的嵌入模型（在推荐列表下方）
    local_only = [m for m in local_emb_models if m["source"] == "local"]
    if local_only:
        local_rows = ['<div style="font-size:12px;color:#888;padding:4px 0;border-top:1px dashed #ddd;margin-top:4px;">━━ 本地自定义 ━━</div>']
        cur_path = cfg.get("embedding",{}).get("model_path","")
        for m in local_only:
            mid = m["model_id"]
            val = m["path"]
            checked = 'checked' if cur_path == val else ''
            dim = f' | {m["dimension"]}维' if m.get("dimension") else ''
            local_rows.append(
                f'<div style="display:flex;align-items:center;gap:10px;padding:6px 0;border-bottom:1px solid #eee;">'
                f'<input type="radio" name="embedding-model" value="{val}" id="emb-local-{mid}" {checked} '
                f'onchange=updateConfig("embedding","model_path",this.value) style="flex-shrink:0;">'
                f'<label for="emb-local-{mid}" style="flex:1;font-size:13px;" title="{val}">'
                f'[嵌入] {mid} <span style="color:#888;font-size:11px;">({m["size_mb"]}MB{dim})</span>'
                f'<span style="font-size:11px;color:#888;margin-left:4px;">(本地)</span>'
                f'</label>'
                f'<span style="font-size:11px;flex-shrink:0;color:#3B6D11;">已下载</span>'
                f'</div>'
            )
        emb_model_html += "\n".join(local_rows)
    ...
    # MiniCPM 已移除——evidence 语义验证改用 NLI 模型（同一模型）
    minicpm_model_html = ""
...
<div class="container">
    ...
    <div style="background:white;border-radius:16px;padding:28px;max-width:480px;width:90%;box-shadow:0 8px 32px rgba(0,0,0,0.2);">
      ...
      <div class="form-group"><label>知识库嵌入模型</label>
        <select id="rule-model" style="width:100%;padding:8px 10px;border:1.5px solid #ddd;border-radius:8px;font-size:14px;">
          ...
          {''.join(f'<option value="{m["path"]}">[嵌入] {m["model_id"]} (本地)</option>' for m in local_only)}
        </select>
  ...
  <!-- NLI 语义 -->
  <div class="card">
    <h2>🏷️ NLI 语义 <span style="font-weight:400;color:#888;font-size:12px;">— 输入校验 + 输出标注，共用同一模型</span></h2>
    <div class="form-row">
      <!-- NLI 输入校验：校验 LLM 输出的 action 中的 evidence 合法性 -->
      <div class="form-group">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
          <label style="font-weight:600;color:#333;white-space:nowrap;">📥 NLI 输入校验</label>
          <label class="toggle-switch {"disabled" if not has_nli else ""}" onclick='if(!hasNLIModel)return;toggleNLIOutput()'>
            <input type="checkbox" onclick="event.stopPropagation();" {"checked" if cfg.get("nli",{}).get("output_enabled", False) else ""} {"disabled" if not has_nli else ""}><span class="toggle-slider"></span>
          </label>
          <span style="font-size:11px;color:#c92a2a;{"display:inline" if not has_nli else "display:none"};">请先下载 NLI 模型</span>
        </div>
        <div style="font-size:12px;color:#888;">校验 LLM 输出 action 中 evidence 的语义一致性</div>
      </div>
      <!-- NLI 输出标注：标注检索结果，增强给 LLM 的上下文 -->
      <div class="form-group">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
          <label style="font-weight:600;color:#333;white-space:nowrap;">📤 NLI 输出标注</label>
        ...
        <div style="font-size:12px;color:#888;">对检索文档做蕴含/中立/矛盾标注，增强 LLM 输入上下文</div>
        <div style="font-size:11px;color:#aaa;margin-top:4px;">输出数量跟随前级</div>
      </div>
    </div>
    <div style="margin-top:12px;">
...
class RAGHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            elif path == "/api/prompt/presets/select":
                data = self._read_body()
                key = data.get("key", "")
                from prompt_manager import set_selected_preset
                if key:
...
"""
            ...
            elif path == "/api/nli/output-toggle":
                cfg = load_config(); nli = cfg.setdefault("nli", {})
                nli["output_enabled"] = not nli.get("output_enabled", False)
                save_config(cfg); self._send_json({"success": True, "enabled": nli["output_enabled"]})

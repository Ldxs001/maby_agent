...
- **移除无效正则 `研究\b`**：中文环境下 `\b` 边界几乎不触发（实测全库零贡献），且该词在正文高频出现存在误判隐患

---

## [2.2.13] - 2026-08-09
### 修复
- **is_header 标记顺序调整：内容探测先行、位置兜底(块0-3)后置**。原实现"先 0-3 兜底 → 后内容探测（跳过已标记块）"导致块 3 命中后不扩展块 4，头部信息被切碎在 3/4 边界时丢失（作者名切到块 4 漏标记）。新实现：0-3 也参与内容探测（命中扩展 +1），位置兜底后置幂等覆盖，布尔标记无冗余，工作量差异可忽略（固定 4 次正则调用）。同步修改 `rag_core.py mark_header_chunks` 与 `backfill_headers.py backfill_kb` 两处
...
## [2.2.11] - 2026-07-31
### 变更
- **README 重写为完整使用手册**：165 → 351 行。新增环境依赖（LLM 后端 + 三类模型自动多源下载）、完整搭建步骤、四启动模式、三端口架构（8765/8766/8767）、知识库管理与 5 切分策略 × 5 守卫、组合式查询 / 多库路由 / 三层推理流水线详解、功能开关矩阵、联网搜索 5 后端配置、记忆系统、提示词与插件系统、35 个外部 API 端点分类、FAQ。面向使用者，无技术实现细节

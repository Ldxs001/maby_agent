...
echo ========================================
echo    Orchestrator - Setup (port %PORT%)
echo ========================================
echo.
...
:: Kill any process still on our port (fixed port => reliable cleanup)
for /f "tokens=5" %%a in ('netstat -ano ^| find ":%PORT% " 2^>nul') do taskkill /f /pid %%a >nul 2>&1

:: Start server (fixed port, background)
start /B "" python main.py --web --port %PORT% --pidfile "%~dp0server.pid"

:: Wait for the port to be listening (up to 15s)
set WAIT=0
:WAIT_LOOP
